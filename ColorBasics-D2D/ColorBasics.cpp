...
int CColorBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    //start the thread with function saverThread that empties the buffer
	//m_saverThreadRunning = true;
	//std::thread task(&CColorBasics::saverThread,this);
...
void CColorBasics::saverThread()
{
	while (m_saverThreadRunning || !m_buffer2.empty())
	{
		m_bufferLock.lock();
		bool isEmpty = m_buffer2.empty();
		...
		while(isEmpty)
		{
			...
			isEmpty = m_buffer2.empty();
		}

		IColorFrame* firstElement = m_buffer2.front();
		m_bufferLock.unlock();

		SaveImages(firstElement);

		m_bufferLock.lock();
		m_buffer2.erase(m_buffer2.begin());
		m_bufferLock.unlock();
...
void CColorBasics::Update()
{
	...
	{
		//m_bufferLock.lock();
		//m_buffer2.push_back(pColorFrame);
		//m_bufferLock.unlock();
	...
	SafeRelease(pColorFrame);
		...
		//m_cFrameCounter++;
		//std::cout << m_cFrameCounter << std::endl;
		//SafeRelease(pColorFrame);
	
}

void CColorBasics::SaveImages(IColorFrame* pColorFrame)
{
		INT64 nTime = 0;
		IFrameDescription* pFrameDescription = NULL;
		int nWidth = 0;
...
void CColorBasics::ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight) 
{
    ...
    if (pBuffer && (nWidth == cColorWidth) && (nHeight == cColorHeight))
    {
       ...
       // m_pDrawColor->Draw(reinterpret_cast<BYTE*>(pBuffer), cColorWidth * cColorHeight * sizeof(RGBQUAD));
        ...
        if (true)//m_bSaveScreenshot)
        {
			IColorFrame* pColorFrame = NULL;
			HRESULT hrr = m_pColorFrameReader->AcquireLatestFrame(&pColorFrame);
			if (SUCCEEDED(hrr))
			{
				INT64 nTime = 0;
			...
			std::string result;
			//stream << nTime;
			stream << m_cFrameCounter;
			stream >> result;
			

            WCHAR szScreenshotPath[MAX_PATH];
			std::wstring widestr = std::wstring(result.begin(), result.end());
			const WCHAR *timestamp =  widestr.c_str();

			//const WCHAR *timestamp = reinterpret_cast<const WCHAR*>(stream.str().c_str());
		
		//	result = "D:\KinectFusion\Dataset";
		//	widestr = std::wstring(result.begin(), result.end());
			WCHAR* pszKnownPath = NULL;
			pszKnownPath = L"D:\\KinectFusion\\Dataset\\rgb";

			//SHGetKnownFolderPath(FOLDERID_Pictures, 0, NULL, &pszKnownPath);
			StringCchPrintfW(szScreenshotPath, _countof(szScreenshotPath), L"%s\\%s.bmp", pszKnownPath, timestamp);
           ...
           // GetScreenshotFileName(szScreenshotPath, _countof(szScreenshotPath));
            ...
            //todo:
            //remove the above command and only save the image to the buffer:
            //
            //m_buffer.push_back(std::pair<BYTE*,const WCHAR*>(reinterpret_cast<BYTE*>(pBuffer),timestamp));
