...
void CDepthBasics::ProcessDepth(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth)
{
    ...
    if (m_pDepthRGBX && pBuffer && (nWidth == cDepthWidth) && (nHeight == cDepthHeight))
    {
        ...
        while (pBuffer < pBufferEnd)
        {
            ...
            BYTE intensity1 = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? (depth / 256) : 0);
			BYTE intensity2 = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? (depth % 256) : 0);

			BYTE intensity = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? depth /(6000 /256) : 0);
            pRGBX->rgbRed   = intensity1;
            pRGBX->rgbGreen = intensity2;
       ...
       m_pDrawDepth->Draw(reinterpret_cast<BYTE*>(m_pDepthRGBX), cDepthWidth * cDepthHeight * sizeof(RGBQUAD));
		//m_pDrawDepth->Draw(reinterpret_cast<BYTE*>(INTENS), cDepthWidth * cDepthHeight * sizeof(BYTE));

        if (true)//m_bSaveScreenshot)
        {
			IDepthFrame* pDepthFrameSource = NULL;
			HRESULT hrr = m_pDepthFrameReader->AcquireLatestFrame(&pDepthFrameSource);
			if (SUCCEEDED(hrr))
			{
				INT64 nTime = 0;
				pDepthFrameSource->get_RelativeTime(&nTime);
			}
			SafeRelease(pDepthFrameSource);
			...
			pszKnownPath = L"D:\\KinectFusion\\Dataset\\depth";
           ...
           // GetScreenshotFileName(szScreenshotPath, _countof(szScreenshotPath));
			...
			//HRESULT hr = SaveBitmapToFile(reinterpret_cast<BYTE*>(INTENS), nWidth, nHeight, sizeof(BYTE) * 8, szScreenshotPath);
